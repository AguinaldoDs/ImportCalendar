...
icon = (r'\\MyLinkedServer\temp\Temp_py\carregar.png')
icon_var = QIcon(icon)
...
def send_to_database(tabelaRetorno):
    ...
    connection_string = 'DRIVER={SQL Server};SERVER=MyLinkedServer;DATABASE=MyDb;UID=USR_MIS;PWD=MIS@321'
...
icon_clear = (r'\\MyLinkedServer\temp\\Temp_py\limpar-limpo.png')
...
#add icon
icon = (r'\\MyLinkedServer\temp\\Temp_py\lupa.png')
...
icon = (r'\\MyLinkedServer\temp\Temp_py\cadastro.png')
icon_tratado = QIcon(icon)
...
def auto_compl_():
    ...
    #path
    conn_str = r"Driver=SQL Server;" f"Server=MyLinkedServer;" r"trusted_connection=yes;"
    ...
    query = (f"""
                select 
                    ...
                    DiaUtil
                from MyDb.geral.calendario with(nolock)
...
icon = (r'\\MyLinkedServer\temp\\Temp_py\lixeira-de-reciclagem.png')
icon_tratado = QIcon(icon)
...
def deleta_casos():
    ...
    consulta_retorno2 = sa.text(f"""
                                delete a 
                                from MyDb.dbo.TabelaFrasesGlobalPy a
    ...
    try:
        ...
        consulta_retorno2 = sa.text(f"""
            SELECT
                ...
                [Var5],
                [Segmentacao]
            FROM MyDb.dbo.TabelaFrasesGlobalPy with(nolock)
    ...
    try:
        ...
        consulta_retorno2 = sa.text(f"""
                     SELECT
                        ...
                        ,[FX10]
                        ,[Data_Importacao]
                    FROM [MyDb].[dbo].[TabelaCalendarioSmsGlobalPy] with(nolock)
                    WHERE mes = '{selected_option_mes}'
                        AND ano = '2024'
                        AND Banco = '{selected_option_banco}'
                        AND CARTEIRA = '{selected_option_carteira}'
...
def deleta_casos_wd2():
    ...
    consulta_retorno2 = sa.text(f"""
                                delete a 
                                from MyDb.dbo.TabelaCalendarioSmsGlobalPy a
                                where a.mes = '{selected_option_mes}'
                                and a.ano = '2024'
                                and a.banco = '{selected_option_banco}'
                                and a.carteira = '{selected_option_carteira}'
    ...
    linhas_paramentro = sa.text(f"""
                                select count(1) 
                                from MyDb.dbo.TabelaCalendarioSmsGlobalPy a with(nolock)
                                where a.mes = '{selected_option_mes}'
                                and a.ano = '2024'
                                and a.banco = '{selected_option_banco}'
                                and a.carteira = '{selected_option_carteira}'
...
# In[82]:
#Apresentação tela 
image_path = (r'\\MyLinkedServer\temp\Temp_py\logo_mis.jpg') #caminho
